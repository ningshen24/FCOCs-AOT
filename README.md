...
## Orange workflow reconciliation

The revised Mouse and Rat Orange workflows now match the final ML settings reported in Table S6 for the previously conflicting fields: NN `hidden_layers_input = 2000`, NN `replicable = True`, and SVM `tol = 0.01`. The revised Rat workflow also contains the final QSTR descriptor `F06[C-N]` in the current saved contexts.

Older historical Orange context strings, including `F01[C-N]`, can still remain embedded in `.ows` widget history. They are retained as provenance only and are explicitly reported by the non-executing audit script; they are not treated as the final Rat ML input. See `docs/ORANGE_WORKFLOW_AUDIT.md` and `models/orange/workflow_vs_reported_discrepancies.csv`.

## Data-quality reconciliation

Repository preparation identified and resolved several cross-file issues:

- Mouse QSTR, **CID 12442**: the dedicated Mouse-QSTR model workbook contains a descriptor-row column shift. The correct descriptor values in the supplementary table and cleaned descriptor matrix are used in the processed CSV. With this correction, the published Mouse QSTR equation reproduces the reported training and external statistics.
- Mouse QSTR, **CID 3026**: the author-corrected supplementary Table S1 now records the experimental endpoint as **1.90**, matching the dedicated Mouse-QSTR model workbook and cleaned descriptor matrix.
- Rat q-RASTR **Q²_LOO = 0.8057** is the confirmed authoritative value. The model specification, reported-performance CSV and repository documentation have been synchronized to 0.8057.
- Table S9 **CID 18725** has no Generalized Jaccard output; the author-corrected supplementary workbook now represents this as `-` rather than a spreadsheet formula error. A workbook error scan finds no remaining `#NAME?`, `#REF!`, `#DIV/0!`, `#VALUE!` or `#N/A` cells.

Full details are in `docs/DATA_CHECKS.md`.
...
If this repository is used before the article receives final bibliographic information, cite the manuscript and the versioned repository release. After acceptance, update `CITATION.cff` with the article DOI and the Zenodo DOI of the archived GitHub release.

## Public release notes

Before article submission or archival release, authors should still record the exact Orange3 and alvaDesc versions if available, confirm final data/code licensing, update repository/Zenodo identifiers and DOI metadata, and archive the accepted repository version on Zenodo. The known CID 12442 source-workbook row issue is documented in `docs/DATA_CHECKS.md`.

## Reference examples used for repository organization

- *Journal of Cheminformatics* author guidelines: https://github.com/jcheminform/jcheminform-author-guidelines
- Recent exposome toxicity prediction paper with GitHub + Zenodo data/code release: https://github.com/idslme/exposome-toxicity-prediction
- Toxicity/imputation study organized into `Code/`, `Data/`, and `Results/`: https://github.com/mowal/Imputation_Paper
